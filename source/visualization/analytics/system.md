...
该模型通常基于 Card{cite}`card1999readings`于1999 年提出的信息可视化模型（{numref}`fig-visualization-analytics-inform_model`），即从原始数据出发，通过“数据转换”“视觉映射”与“视图转换”等阶段，最终完成用户与可视化视图的交互过程。
